...
Modifiers are written `Ctrl`, `Alt` and `Shift`. In Vi mode a terminal that
does not send `Alt` can press `Esc` first and then the key; Emacs and Helix
read the two as separate keys.
...
| Key | Action |
| --- | --- |
| `Up`, `Ctrl-p` | Move up in an open menu, otherwise move one line up, walking history backwards from the first line |
| `Down`, `Ctrl-n` | Move down in an open menu, otherwise move one line down, walking history forwards from the last line |
...
An unbound `Alt-<char>` is read as `Esc` followed by `<char>` in every vi mode,
the meta convention of readline and zsh: `Alt-k` from insert recalls the
previous line. Binding the `Alt` chord itself is the per-key opt-out, which is
why `Alt-<` and `Alt->` keep their common-set meaning.
...
| --- | --- |
| `Enter` | Submit the input, or insert a newline if it is incomplete |
| `Esc` | Switch to normal mode, stepping the cursor back onto the last typed grapheme |

The cut and case commands are not bound here: `Ctrl-k`, `Ctrl-u`, `Ctrl-y`,
`Ctrl-t`, `Alt-d`, `Alt-u`, `Alt-l` and `Alt-c` belong to Emacs mode. Being
unbound, the `Alt` ones fall to the meta convention below, so `Alt-u` undoes
and `Alt-d` leaves insert and starts a `d` operator. `Ctrl-w` deletes a word
...
| `?` | Search the history and switch to insert mode |
...
The normal-mode commands apply here too, except `u` and `o`/`O`, which mean
something else in visual. This table lists the keys that differ.
...
`Ctrl-Home`/`Ctrl-End` with `Alt-<`/`Alt->` follow `gg`/`ge`. `Up`, `Down`,
