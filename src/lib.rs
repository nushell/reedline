...
//! use reedline::{default_emacs_keybindings, EditCommand, Emacs, Reedline, ReedlineEvent};
//!
...
//! ## Crate features
//!
//! - `clipboard`: Enable support to use the `SystemClipboard`. Enabling this
//!   feature will return a `SystemClipboard` instead of a local clipboard when
//!   calling `get_default_clipboard()`.
...
//! - `sqlite`: Provides the `SqliteBackedHistory` to store richer information
//!   in the history. Statically links the required sqlite version.
//! - `sqlite-dynlib`: Alternative to the feature `sqlite`. Will not statically
//!   link. Requires `sqlite >= 3.38` to link dynamically!
//! - `external_printer`: **Experimental:** Thread-safe `ExternalPrinter` handle
//!   to print lines from concurrently running threads.
//!
...
//! )
//!
//! - General editing functionality, that should feel familiar coming from other
//!   shells (e.g. bash, fish, zsh).
...
//! - History with interactive search options (optionally persists to file, can
//!   support multilple sessions accessing the same file)
//! - Fish-style history autosuggestion hints
//! - Undo support.
//! - Clipboard integration
//! - Line completeness validation for seamless entry of multiline command
//!   sequences.
//!
...
//! - [ ] Visual selection
//! - [ ] Smooth experience if completion or prompt content takes long to
//!   compute
//! - [ ] Support for a concurrent output stream from background tasks to be
//!   displayed, while the input prompt is active. ("Full duplex" mode)
//!
...
// Reexport the key types to be independent from an explicit crossterm
// dependency.
pub use crossterm::{
